...
lst_t *add_node(lst_t **head, const char *str, int num)
{
	...
	if (!new_head)
		return (NULL);
	_memset((void *)new_head, 0, sizeof(lst_t));
	new_head->num = num;
	if (str)
	{
...
lst_t *add_node_end(lst_t **head, const char *str, int num)
{
	...
	if (!new_node)
		return (NULL);
	_memset((void *)new_node, 0, sizeof(lst_t));
	new_node->num = num;
	if (str)
	{
 ...
 */
int delete_node_at_index(lst_t **head, unsigned int index)
{
